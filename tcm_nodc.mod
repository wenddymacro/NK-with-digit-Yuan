...
% This Dynare code simulates a 2-country NK model w/o CBDC.
...
mustar ximstar sigmstar mss mstarss
...
tb=bH+rer*bF-r(-1)/pi*bH(-1)-rer*rz(-1)/piz*bF(-1)+kappaD/2*rer*(bF-bFss)^2-(1-n)/n*kappaD/2*(bHz-bHzss)^2+m-xim*m(-1);
tbz=bFz+bHz/rer-rz(-1)/piz*bFz(-1)-1/rer*r(-1)/pi*bHz(-1)+kappaD/(2*rer)*(bHz-bHzss)^2-n/(1-n)*kappaD/2*(bF-bFss)^2+mstar-ximstar*mstar(-1);
